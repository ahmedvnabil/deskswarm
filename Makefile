...
up:  ## build and start the dashboard (no configuration needed)
	$(COMPOSE) up -d --build
	@echo
	@echo "  deskswarm is starting on http://localhost:$(PORT)"
...
backup:  ## back up every machine's home directory now
	@curl -fsS http://localhost:$(PORT)/api/v1/computers \
		| python3 -c 'import json,sys;[print(c["id"]) for c in json.load(sys.stdin)["data"]]' \
		| while read id; do \
			curl -fsS -X POST http://localhost:$(PORT)/api/v1/computers/$$id/backups \
...
doctor:  ## check the things that usually go wrong
	...
	@echo "dashboard:"; curl -fsS http://localhost:$(PORT)/health 2>/dev/null | sed 's/^/  /' || echo "  not running"
	@echo "guards:"; curl -fsS http://localhost:$(PORT)/api/v1/guards 2>/dev/null \
		| python3 -c 'import json,sys;g=json.load(sys.stdin)["data"];print(f"  {g[\"memory_available_mb\"]} MB free ({g[\"memory_source\"]}), {g[\"disk_free_gb\"]} GB disk");[print("  !",w) for w in g["warnings"]+g["blocking"]]' 2>/dev/null || echo "  n/a"
