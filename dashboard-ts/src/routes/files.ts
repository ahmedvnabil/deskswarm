...
import { providerFor, HomePathMissing, PathOutsideHome } from "./../providers";
...
files.get("/api/v1/computers/:id/files", async (c) => {
  ...
  try {
    return ok(c, { path: rel, entries: await providerFor(comp).listHome(comp.slug, rel) });
  } catch (err: any) {
    if (err instanceof PathOutsideHome) return fail(c, String(err.message));
    if (err instanceof HomePathMissing) {
...
files.post("/api/v1/computers/:id/files", requireToken, async (c) => {
  ...
  try {
    const path = await providerFor(comp).uploadToHome(comp.slug, relDir, basename(uploaded.name), data);
...
files.get("/api/v1/computers/:id/files/download", async (c) => {
  ...
  try {
    const [blob, name] = await providerFor(comp).downloadFromHome(comp.slug, rel);
...
files.get("/partials/computers/:id/files", async (c) => {
  ...
  try {
    entries = await providerFor(comp).listHome(comp.slug, rel);
  } catch (err: any) {
